...
#if defined(ESP32)
    d["txpower_dbm"] =
        WiFi.getTxPower() > 0 ? String((float)WiFi.getTxPower() / 4.0f, 1)
                              : "n/a";  // enum codes are quarter-dBm units
#endif
    d["down_since_s"] =
...
void NetServices::loop() {
  HCS_MARK("net.rb");
  otaRollbackTick();
  bulletproofWifiTick();  // active re-association + wedge self-heal (was orphaned)
